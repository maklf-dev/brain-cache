...
- `git restore` : discards unstaged changes; recovers the last committed version
...
- `git revert` : creates a new commit that reverses the old one
    - `git revert HEAD` : prev commit exists, but new commit cancels it; recover the version before last committed version
- `git reset` : for recover the prev version
    - `git reset --soft` : undo the last commit to staged step; last committed version turn to staged version
    - `git reset --mixed` (default; === `git reset`) : undo the last commit to unset step; remove commit and stage but keeps the file changes 
- `git stash`
    - `git stash push -m ""` : save and add commit temporarily; safe for moving between branches at the middle of work
...
- **branch** : an sticky note that points at a version of main
...
- **checkout** : moves head safely, without any changes. just moving viewpoint
- **reset** : move main branch to point at specific commit. ahead commit still exist, but orphaned
    - **soft** : moves branch, but staging area and wd stay unchanged. for when you want to combine commits into one
    - **mixed** : default; move branch and change staging area. for when you want to restage a commit differently; split it to multiple commit
    - **hard** : **DANGER ZONE!**; move branch, reset staging area, reset working directory. Uncommit work gone.
- **revert** : creates new commit and remove latest changes on that commit. actually you copy a version of your commit, before applying changes to it
- **rebase** : when you have some branches and different version, you have two choice:
    - **merge** : which merge all the the changes to final version and you have one final commit with all the changes
...
- **other branches** : for work on something safely in project, without ruin the real version
    > when you want to add something to your project, you first make a branch;then you change what ever you want on that version, without any harm to your real project when all your changes are done, and your test passed, you can safely merge that branch to main
...
| file committed but not pushed | `git commit --amend -m "<newCommit>"` |
| file committed but need to add file to the same commit | `git commit --amend --no-edit` |
| file committed and push but need to recover | `git revert` |
